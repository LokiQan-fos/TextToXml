...
- `src/Kape22Importer/Persistence/AscoLsiDbContext.cs:15-37` -- 12 `DbSet`s in total; the extended A-2 assertions enumerate the 9 Story 4.1 dispatch tables plus `ConsignesRows` (`Kape22Rows` and `LogCommandeRows` were already asserted by the two tests before this story).
...
- [x] [Review][Patch] Code Map miscounts `AscoLsiDbContext`'s DbSets as 10 (`Kape22Rows + 9 downstream`) — the referenced line range declares 12 DbSets; the A-2 assertions actually cover 9 dispatch tables + `ConsignesRows`, and `LogCommandeRows` is uncounted either way [_bmad-output/implementation-artifacts/spec-4-9-hardening-epic-4.md Code Map / src/Kape22Importer/Persistence/AscoLsiDbContext.cs:15-37]
- [x] [Review][Defer] A-5's pre-check reports only the first colliding `CodeOperation` group (`.FirstOrDefault(group => group.Count() > 1)`) — a bundle with two independent collisions only ever surfaces one; this matches the frozen spec's own "a collision produces one ConversionError" boundary, so widening it would need renegotiation. Already logged in deferred-work.md by the prior review pass [src/Kape22Importer/Persistence/Kape22Persister.cs:121] — deferred, pre-existing
- [x] [Review][Defer] The missing-Coulee and Consignes-collision checks in `PersistMapped` are sequential early-returns, not accumulated — a Fichier failing both only ever reports the missing-Coulee error, matching `Kape22Persister`'s established one-check-at-a-time pattern. Already logged in deferred-work.md by the prior review pass [src/Kape22Importer/Persistence/Kape22Persister.cs:91,110] — deferred, pre-existing
- [x] [Review][Defer] No test exercises a blank/whitespace-only `OF` or `Coulee` Champ through the new unconditional `entity.OF.Trim()`/`entity.Coulee.Trim()` — confirmed safe by inspection (the DTO fields default to `string.Empty`, never null, so no NRE risk) but the post-trim required-field-blank path isn't directly asserted for OF/Coulee specifically, only the generic `RequiredFieldCheck` pattern for other fields [src/Kape22Importer/Kape22Mapper.cs:112] — deferred, pre-existing
- [x] [Review][Defer] A-5's REJETÉ message cites only the raw colliding `CodeOperation` value, not which two sections collided (e.g. Chutage vs Decoupe) — an operator reading `L_D_LOG_COMMANDE` has to cross-reference the source Fichier by hand [src/Kape22Importer/Persistence/Kape22Persister.cs:124] — deferred, pre-existing
