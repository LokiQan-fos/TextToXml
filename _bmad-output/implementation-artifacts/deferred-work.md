...
- source_spec: `annexe-mapping-dispatch-epic4.md` § L_D_CONSIGNES
  summary: `ConsignesMapper` leaves `TypeConsigne`/`ConsigneGPAO` at the same constant (`0`/`false`) for every section (à_clarifier per the Story 4.2 annex, see the entry above). `L_D_CONSIGNES`'s own natural key is `(OF, CodeOperation, TypeConsigne, ConsigneGPAO)` (Story 4.1 doc comment), so two sections sharing the same `CodeOperation` value for one OF would produce two rows with an identical key. Not reproducible against any of the 10 reference `P60/` fixtures (each section's `CodeOpe*` field is a distinct KAPE22 Champ with its own value), so left as a documented risk rather than a mapper-level guard, which would need cross-section knowledge this pure per-source mapper does not have.
