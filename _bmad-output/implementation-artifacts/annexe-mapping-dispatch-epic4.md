...
| Colonne | Statut | Source / Règle | Scale |
| --- | --- | --- | --- |
| CodeConsigne | sourcée | KAPE22, champ variable selon la section (ex. `CodeConsigneLingot` pour ConsignesLingot, `CodeConsigneChutage` pour ConsignesChutage — cf. OrdreFabrication.xml, sous-objets `ConsignesL/C/D/R/P/S/PM`) ; écart accepté (2026-09-24) : espaces de fin supprimés, le legacy les conserve (ex. `"00 1        "`) | - |
...
| ConsigneGPAO | règle | Deux lignes par consigne, mêmes codes à l'import (`AddOrModifyConsigne`, OrdreDeFabricationManager.cs:1362-1435, un seul `codeconsigne` pour les deux) : `true` = valeur reçue du GPAO, jamais retouchée ; `false` = copie de travail, retouchée ensuite par les opérateurs (commande MCC, ex. « Changement de Consigne P900: 394 --> 999 »). Sémantique corrigée par sprint-change-proposal-2026-09-24.md (celle du 2026-09-23 était inversée). `ConsignesMapper` ne produit encore que les lignes `true` ; la ligne `false` est l'objet de la Story 4.13 | - |
| LibelleConsigne | règle | Story 4.12 : port pur de `LibelleConsigneController.GetLibelle` (`Lsi.Net/Ascometal.LSI.DAL/LibelleConsigneController.cs:14-285`) dans `LibelleConsigneResolver`, sur un instantané des 13 tables `L_P_CONSIGNES_*` chargé une fois par Fichier (`ConsigneReferenceData`) ; type 22 lit aussi `ProfilProduit`/`DiametreProduit` de l’OF et `H2Coulee` des Pits ; libellé vide ou exception = `?` (OrdreDeFabricationManager.cs:1405-1416) ; Story 4.13 : lignes `ConsigneGPAO=false` de type 13 (6 sections décodées) et 24 (XP1) = libellé composite, port exact de `OrdreFabrication.BuildLibelleConsigne` (OrdreFabrication.cs:700+) ; les lignes `true` de ces types restent `?` | - |
| OF | sourcée | KAPE22.OF | - |
